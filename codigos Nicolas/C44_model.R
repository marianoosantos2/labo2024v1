#  PROMEDIO DE MEDIAS DE VALORES MES ANTERIOR/POSTERIOR.
...
for (i in seq_len(nrow(resultados_filtrados))) {
  ...
  media_var_mes_ant <- valor_cliente_mes_ant[, .(mean_value = mean(valor, na.rm = TRUE))]
  media_var_mes_pos <- valor_cliente_mes_pos[, .(mean_value = mean(valor, na.rm = TRUE))]

  prom_med <- (media_var_mes_ant$mean_value + media_var_mes_pos$mean_value) / 2
  print(prom_med)
