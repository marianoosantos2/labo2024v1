...
for (i in seq_len(nrow(resultados_filtrados))) {
  ...
  set.seed(100297)
  merged_data[, valor_random := mapply(runif, 1, min=pmin(valor_ant, valor_pos), max=pmax(valor_ant, valor_pos))]
  data[, (var_var) := as.numeric(get(var_var))]
  data[foto_mes == var_mes & numero_de_cliente %in% clientes_com, (var_var) := merged_data[.SD, on = .(numero_de_cliente), x.valor_random]]
} 
