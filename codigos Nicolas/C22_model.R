...
for (i in seq_len(nrow(resultados_filtrados))) {
  ...
  res_mes <- data[foto_mes == var_mes, .(Total = .N, Ceros = sum(get(var_var) == 0, na.rm = TRUE))]
  res_mes_ant <- data[foto_mes == var_mes_ant, .(Total = .N, Ceros = sum(get(var_var) == 0, na.rm = TRUE))]
  res_mes_pos <- data[foto_mes == var_mes_pos, .(Total = .N, Ceros = sum(get(var_var) == 0, na.rm = TRUE))]
